...
class AddListing extends React.Component {
    ...
    handleSubmit = async () => {
        ...
        this.setState({ loading : false, success : true, listingId : res.listingId  })
    }
        ...
        return (
            <div style={{ display: 'flex', alignItems: 'flex-start',  flexDirection: 'column', padding: '20px' }}>
                ...
                <div style={{ borderBottom: '1px solid rgb(31,33,48)', width: '20%', textAlign: 'left', marginTop: '3em' }}>       
                    ...
                    <h3 style={{ fontSize: '18px', marginBottom: '0.5em'}}>Contact Photos: {this.props.photos.contact_photos.length == 0 && <span style={{ color: 'red', fontSize: '16px'}}>No photos provided</span>}</h3>
                    {
                ...
                <h3 style={{ fontSize: '18px', marginBottom: '0.5em'}}>Cover Photos: {this.props.photos.cover_photos.length == 0 && <span style={{ color: 'red', fontSize: '16px'}}>No photos provided</span>}</h3> 
                {
                    <div style={{ display: 'flex',  height: '270px', width: '100%',  overflowX: 'auto', flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgb(242,242,242)' }}>
                        {this.props.photos.cover_photos.map((file, i) => {
                            return (
                                <div style={{ minWidth: '225px', maxWidth: '225px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', marginTop: '0.5em'}}>
                                    <Image src={URL.createObjectURL(file)} style={{ width: '200px', height: '200px'}} />
                                    <p style={{ marginTop: '0.5em', fontSize: '12px'}}>{file.name}</p>
            ...
            <h3 style={{ fontSize: '18px', marginBottom: '0.5em', marginTop: '1em'}}>Site Photos: {this.props.photos.site_photos.length == 0 && <span style={{ color: 'red', fontSize: '16px'}}>No photos provided</span>}</h3>
                ...
                <div style={{ width: '100%', height: '100px', display: 'flex', flexDirection: 'column',  alignSelf: 'center'}}>
                    ...
                    {this.state.success && <div> <span>Success</span><br></br><Link to={"/listing/"+this.state.listingId}>Go to your listing </Link> </div>}
                </div>
            </div>
        )
    }
