...
export const getNearbyPlaces = (map, center, type) => {
    let lat = center.coords.lat
    let lng = center.coords.lng
    let request = {
        location: [],
        rankby: 'distance',
        type: type
    ...
    service.findPlaceFromQuery(request, (res, stat) => {
        if (stat === google.maps.places.PlaceServiceStatus.OK) {
            console.log(res)
        }
    })
}
