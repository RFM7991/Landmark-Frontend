...
class SimpleMap extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      boundaries : getPoints(this.props.center, 0.402),
      tradezone : undefined
    }  
      ...
      if (this.state.map != undefined) {
       ...
       /* 
        for (var i =0; i < this.state.markers.length; i++) {
          console.log('markers', this.state.markers[i].marker)
  ...
  this.state.markers.set(id, {marker: marker, place: place})
    ...
    this.state.boundaries.forEach(e => {
      var marker = new google.maps.Marker({
        key: 0,
        position: e,
        map: map,
...
const mapStateToProps = createSelector(
  ...
  stateSelector,
  tractSelector,
  (places, active_place, address, business_type, zip, state, tract) => ({
    places, 
    active_place,
    address,
    business_type,
