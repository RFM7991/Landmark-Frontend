...
class SimpleMap extends Component {
  ...
  async checkIsCity() {
    ...
    let population = data[0].B01003_001E
    console.log('RFM', population)
    return Number(population) >= 1000000
  }
  clearCartography() {
...
handleSwitch = (checked) => {
      ...
      var content ='<h3>Your ' + this.props.business_type.toString() +'</h3>'
       ...
       if (this.state.isCity != undefined) {
         ...
         if (this.state.isCity) {
           //  0.220 : 0.402);
            const circle1 = renderCircle(220, "#00ff00", map, center) //0.0125 miles
            const circle2 = renderCircle(402, "#ffff00", map, center) // 0.25 miles
             // to-do cleanup, add label
           let points = getPoints(center, 0.220)
           renderMarker('circle1', points[0], map, '0.0125 miles', GREEN_MARKER)
           points = getPoints(center, 0.402)
           renderMarker('circle1', points[0], map, '0.25 miles', YELLOW_MARKER)

         } else if (!this.state.isCity) {
          const circle1 = renderCircle(402, "#00ff00", map, center) // 0.25 miles
          const circle2 = renderCircle(804, "#ffff00", map, center) // 0.5 miles
          // to-do cleanup, add label
           let points = getPoints(center, 0.402)
           let marker1 = renderMarker('circle1', points[0], map, '0.25 miles', GREEN_MARKER)
           points = getPoints(center, 0.804)
           renderMarker('circle1', points[0], map, '0.5 miles', YELLOW_MARKER)
         }
       }
  ...
  <div className='map-control_bar'>
        ...
        <button className='map-control_button'  onClick={this.loadMorePlaces} className='map_buttons' style={{float: 'left'}}>
         ...
         <Form.Control value={this.state.places_count} as="select" name="places_count" onChange={this.onPlacesCountChange} style={{width: '100px', textAlign: 'center', display: 'inline', marginLeft: '75px', marginRight: '75px'}}>
                        <option>20</option>
                        <option>40</option>
                        <option>60</option>
          </Form.Control>
