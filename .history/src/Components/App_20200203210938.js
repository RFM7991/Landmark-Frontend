...
import { setRecentSearches } from '../Requests/users-requests' 
  ...
  //  localStorage.clear()
    ...
    setInterval(async () => {
      ...
      if (this.props.user._id != -1) {
        ...
        if (searches != null) {
          console.log('COMPARE', searches == this.state.searches, searches, this.state.searches)
          if (searches != this.state.searches) {
             this.setState({ searches : searches}) 
            
            searches = JSON.parse(searches)
            console.log('SEARCHES FOR UPLOAD' searches)
            await setRecentSearches(this.props.user._id, new Map(searches)).then(data => console.log('SET RECENTS', data))
          }
        }
