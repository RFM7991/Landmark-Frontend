...
class RecentSearches extends React.Component {
    ...
    async componentDidMount() {
        ...
        let res = await getListingByUserId(this.props.user._id)

        console.log("MY_LISTINGS", res)
    ...
    render() {
        ...
        return (
            ...
            {!this.state.loading && 
              ...
              {!this.state.loading && this.props.listings.length == 0 &&  <div style={{ width: '100%', height: '80vh', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: darkBg, color : 'white'}}><p>Sorry, there are no listings yet for these parameters</p></div>}
            </div>    
       )
    }
 }
