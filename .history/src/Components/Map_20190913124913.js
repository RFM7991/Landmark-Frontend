  ...
  updateRadii() {
    ...
    if (this.state.isCity != undefined) {
      ...
      if (this.state.isCity) {
        //  0.220 : 0.402);
         const circle1 = renderCircle(220, "#00ff00", this.state.map, this.state.center) //0.0125 miles
         const circle2 = renderCircle(402, "#ffff00", this.state.map, this.state.center) // 0.25 miles
          // to-do cleanup, add label
        let points = getPoints(this.state.center, 0.220)
        renderMarker('circle1', points[0], this.state.map, '0.0125 miles', GREEN_MARKER)
        points = getPoints(this.state.center, 0.402)
        renderMarker('circle1', points[0], this.state.map, '0.25 miles', YELLOW_MARKER)

      } else if (!this.state.isCity) {
       const circle1 = renderCircle(402, "#00ff00", this.state.map, this.state.center) // 0.25 miles
       const circle2 = renderCircle(804, "#ffff00", this.state.map, this.state.center) // 0.5 miles
       // to-do cleanup, add label
        let points = getPoints(center, 0.402)
        let marker1 = renderMarker('circle1', points[0], this.state.map, '0.25 miles', GREEN_MARKER)
