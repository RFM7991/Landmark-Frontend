...
import {pushRecentSearch} from '../Requests/users-requests'
...
class LookUpForm extends React.Component {
    ...
    async onHandleSubmit() {
        ...
        if (this.props.address.toString().length > 0) {
            ...
            console.log('999', this.props.data_range)
    ...
    onUpdateAddress = async (address) => {
        ...
        if (this.props.user_id != -1) {
            pushRecentSearch(this.props.user._id, address)
            let searches = JSON.parse(localStorage.getItem('recentSearches'))

            // keep last 25 
            if (searches.length > 25)
            searches = searches.slice((searches.length) - 25, searches.length-1)

            console.log('2221', searches, typeof searches)
            if (searches == null) searches = new Map()
            else searches = new Map(searches)
            console.log('2222', searches, typeof searches)
         //   else searches = JSON.parse(searches)
            searches.set(address.place.place_id, {location: address, date : new Date()})
            localStorage.setItem('recentSearches', JSON.stringify(Array.from(searches.entries())))
            console.log('222', localStorage.getItem('recentSearches'))
        }
...
const mapStateToProps = createSelector(
    ...
    (user, business_type, ready, address, isCity, tradeZone_bounds, data_range) => ({
        user,
        business_type,
        ready,
        address,
