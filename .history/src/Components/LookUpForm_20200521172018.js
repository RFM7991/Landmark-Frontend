    ...
    render() {
        ...
        return (
            <div className="lookup-form-block">
                ...
                <Form.Group controlId="formGridZip" style={{ backgroundColor: 'red'}}>
                    <Form.Label style={{color: textPrimary}} style={{color: textPrimary}}>Your Proposed Business Type</Form.Label>
                    <Form.Control as="select" name="business_type" value={this.state.business_type} onChange={this.onBusinessFormChange} style={{ width: '40%'}}>
                            {BUSINESS_TYPES.map((e,i) => {
                                return <option key={i}>{e.replace(/_/g, ' ')}</option>
                             })}
                     </Form.Control>
                    </Form.Group>
                <AutoCompleteBar  style={{ backgroundColor: 'red'}} urlParams={this.props.urlParams} addressFunction={this.getAddress}/>
                </div>
