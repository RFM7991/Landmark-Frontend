...
class SimpleMap extends Component {
  ...
  loadMorePlaces(index, numPlaces, hasCallback, callback) {
        let cached_places = Array.from(this.state.places_cache.get(this.state.business_type).values())
        if (cached_places.length > index) {
          console.log('pull from cache', index, cached_places.length, cached_places.slice(index, index + numPlaces))
          var morePlaces = this.props.places.concat(cached_places.slice(index, index + numPlaces))
          this.setState({places: morePlaces}, () => {
      ...
      switch (event.target.value) {
        ...
        case '40':
            if (this.props.places.length < 40) {
              this.loadMorePlaces(20, 20, false)
            } else {
              var places = this.props.places
        ...
        case '60':
            if (this.props.places.length < 40) {
              this.loadMorePlaces(20, 20, true, ()=> {
                console.log('load MORE places')
                this.loadMorePlaces(40, 20, false)
              })
  ...
  updateRadii() {
      ...
      if (this.props.isCity) {
        ...
        let circle2Distance = 1608 // 0.5 mi
         const circle1 = renderCircle(circle1Distance, "#00ff00", this.state.map, this.state.center) 
         const circle2 = renderCircle(circle2Distance, "#ffff00", this.state.map, this.state.center) 
          // to-do cleanup, add label
        let points = getPoints(this.state.center, circle1Distance/10)
        renderMarker('circle1', points[0], this.state.map, '0.5 miles', YELLOW_MARKER)
        points = getPoints(this.state.center, circle2Distance/10)
        renderMarker('circle1', points[0], this.state.map, '0.5 miles', YELLOW_MARKER)
        ...
        // to-do cleanup, add label
          let points = getPoints(this.state.center, circle1Distance/10)
          renderMarker('circle1', points[0], this.state.map, '3 miles', GREEN_MARKER)
          points = getPoints(this.state.center, circle2Distance/10)
  ...
  onHandleAerial = () => {
    ...
    if (this.state.aerial) {
      this.setState({ zoom : 18, center : this.props.center, mapType : google.maps.MapTypeId.SATELLITE })
