...
class SimpleMap extends Component {
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.transportation.showSubways != this.props.transportation.showSubways) {
      if (this.props.transportation.showSubways) {
          this.updatePlacesFromRedux('subway_station', 'subway station', 20)
      }
    }
  }
  ...
  updatePlacesFromRedux = async (type, display_value, count) => {
    if (type == 'hotels_/lodging') type = 'lodging'
    else if (type == 'all') type = 'establishment'
    ...
    if (type == 'none') {
      ...
      this.setState({places_count : count, 
        poi : display_value, 
        business_type : type,
        business_type_option : 'none'
      })
