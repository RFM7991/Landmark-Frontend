...
import { activeSelector, addressSelector, businessTypeSelector, dataRangeSelector, placesSelector, stateSelector, tractSelector, zipSelector, isCitySelector, tradeZoneBoundsSelector, transportSelector } from '../Reducers/selectors'
...
import { updateTransporation} from '../Actions/tradeZoneBoundaries-actions'
...
loadNearbySubways = () => {
  ...
  getNearby(this.props.address, type, async (data, token) => {
      ...
      console.log('load subways', subway_data)
      let object = {}
      subway_data.forEach((e, i) => {
        if (e.data != undefined)
          object[e.name] = e.data.TOTAL_ENT
      })
...
generateStreetViewMarkers = () => {
 ...
 //   console.log('map state', this.state)
  ...
  <div className='map-control_bar'>
    <div style={{ display : 'flex', alignItems: 'center', marginLeft: '2.5%'}}>
      <Button variant="dark" onClick={this.onHandleCenter}> 
        Center
      </Button>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center',  marginLeft: '10px'}}>
         <div style={{display: 'flex', flexDirection: 'column', alignItems:'center'}}>
          <p style={{textAlign: 'center', marginBottom: '0'}}> Street View on/off</p>
         <SliderSwitch checked={this.state.siteView} switchFunction={this.onHandleSite}></SliderSwitch>
         </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center',  marginLeft: '10px'}}>
        <div style={{display: 'flex', flexDirection: 'column', alignItems:'center'}}>
          <p style={{textAlign: 'center', marginBottom: '0'}}> Overlay on/off</p>
        ...
        <div style={{ display: 'flex', alignContent: 'center'}}>
         <div>
          <p style={{textAlign: 'center', marginBottom: '0%'}}> Businesses</p>
         <Form.Control value={this.state.business_type_option} as="select" name="business_type" onChange={this.onBusinessFormChange} style={{width: '150px', textAlign: 'center', display: 'inline', maxMenuHeight: '200px'}}>
          ...
          <div>
          <p style={{textAlign: 'center', marginBottom: '0%'}}> Places of Interest</p>
