...
class DemographicsPanel extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            zipDisabled : false,
            zipHeader : 'Zip',
            zipAccordianKeys : ['0', '0', '0', '0'],
            tzAccordianKeys : ['0', '0', '0', '0'],
            accordianKeys :  ['0', '0', '0', '0']
         }
   ...
   onUpdateDataRange(event) {
    ...
    if (val === 'zip') {
        ...
        this.setState({ zipVariant: 'light', tzVariant: 'dark', accordianKeys : this.state.zipAccordianKeys})
    }
    else if (val == 'tradeZone') {
        this.props.onUpdateDataRange(TRADE_ZONE)
        this.setState({zipVariant: 'dark', tzVariant: 'light', accordianKeys : this.state.tzAccordianKeys})
    }
   }
   ...
   async componentDidUpdate(prevProps) {
    ...
    async componentDidMount() {
        this.setState({ accordianKeys : this.state.zipAccordianKeys})
    ...
    async getTradeZoneData() {
        ...
        if (this.props.address.isNewEntry) {
           this.loadNewTradeZoneStats()
            ...
            // set internal state
            this.setState({tradeZoneStats : data[0].stats, statsLoaded : true, tzAccordianKeys : ['0', '1', '2', '3' ]})
            let stats = {
    ...
    loadNewTradeZoneStats = async () => {
        ...
        // set internal state
        this.setState({tradeZoneStats : data, statsLoaded : true, tzAccordianKeys : ['0', '1', '2', '3' ]})
        let stats = {
            zip : this.props.stats.zip,
            tradezone : data
        }
           ...
           <div className={this.props.orientation} style={{backgroundColor: lightBg, display: 'flex', flexDirection: 'column'}}>
            ...
            <div style={{ flexGrow: 1, overflowY: 'auto', backgroundColor: lightBg}}>
               <Accordion defaultActiveKey='0'>
                ...
                <Card style={{backgroundColor: lightBg}}>
                    <Accordion.Toggle as={Card.Header} eventKey={this.state.accordianKeys[1]} style={{color: 'whitesmoke', backgroundColor: darkBg, marginTop: '3px', fontWeight: 'bold'}}>
                    Social
                    </Accordion.Toggle>
                ...
                <Card style={{backgroundColor: lightBg}}>
                    <Accordion.Toggle as={Card.Header} eventKey={this.state.accordianKeys[2]} style={{color: 'whitesmoke', background: darkBg, fontWeight: 'bold'}}>
                    Household Income
                    </Accordion.Toggle>
                ...
                <Card style={{backgroundColor: lightBg}}>
                    <Accordion.Toggle as={Card.Header} eventKey={this.state.accordianKeys[3]} style={{color: 'whitesmoke', background: darkBg, fontWeight: 'bold'}}>
                    Age
                    </Accordion.Toggle>
