...
class DemographicsPanel extends React.Component {
   ...
   onUpdateDataRange(event) {
    ...
    if (val === 'zip') {
     ...
     getData () {
       getAllStats(this.props.lat, this.props.lng, 'zip').then(data => this.setState({ zipStats : data, currentStats : data}))
       getAllStats(this.props.lat, this.props.lng, 'tradezone').then(data => this.setState({tradeZoneStats : data}))
