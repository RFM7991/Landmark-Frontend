...
class SimpleMap extends Component {
  ...
  async loadDefaultPlaces() {
    ...
    let response = await getNearby(this.props.address, this.props.business_type=> {
  ...
  async loadDefaultPlaces() {
    console.log('Loading places')
    await getNearby(this.props.address, this.props.business_type)
    .then((response) => {
      let token = response.headers.get('pragma')
      let data = response.json()
      console.log(data, token)
      var morePlaces = this.props.places.concat(data)
      this.setState({places: morePlaces})
      this.setState({token: token}, () => this.onUpdatePlaces(data)) 
    })
  }
