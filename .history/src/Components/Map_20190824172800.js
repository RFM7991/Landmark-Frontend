...
class SimpleMap extends Component {
      ...
      if (this.props.data_range == ZIP) {
        ...
        if (this.state.cartography.tradezone !== undefined) {
          this.state.cartography.tradezone.forEach(featureSets => {
            ...
            featureSets.forEach(featureSet => this.state.map.data.addGeoJson(featureSets[0]))
          })
       } else {
  ...
  loadCartography() {
    ...
    getTractCartography('nj', '021400').then((data) => {
      console.log(data[0])
      this.state.map.data.addGeoJson(data[0])
    })
