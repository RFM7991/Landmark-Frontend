...
import { getBlockGroups, getZipCodes } from '../Helpers/geojsonReader'
...
class SimpleMap extends Component {
  ...
  console.log('CURRENT_BOUNDS', this.props.tradeZone_bounds, typeof this.props.tradeZone_bounds)
    ...
    let data;
      ...
      if (this.props.geo_unit == 'zip') {
         data = getZipCodes(this.props.tradeZone_bounds, this.props.address.state)
      } else {
  ...
  updateRadii() {
      ...
      let counter = 1;
            let divisor = 10;
            let increment = 4.828 / divisor
            for (let i=1; i <= divisor; i++) {
              console.log('what',  i*increment, counter*10)
                points = points.concat(getPoints(this.state.center, i*increment, counter*10 ))
                counter+=1
            }
      ...
      let outerRing = getPoints(this.state.center, 8.1, 80)
      outerRing.forEach((e,i) => {
        renderMarker(i, e, this.state.map, e.lat + ', ' + e.lng, RED_MARKER)
      })
    }
