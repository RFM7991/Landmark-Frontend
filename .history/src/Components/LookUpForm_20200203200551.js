...
class LookUpForm extends React.Component {
    ...
    onUpdateAddress = async (address) => {
        ...
        if (this.props.user_id != -1) {
            ...
            let arr = localStorage.getItem('recentSearches')
            if (arr == undefined) arr = []
            arr.push({uid: this.props.user._id, location: address})
            localStorage.setItem('recentSearches', JSON.stringify(user))
        }
