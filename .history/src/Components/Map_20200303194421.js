...
import blockdata from '../geojson/block-group.json'
...
class SimpleMap extends Component {
  ...
  onUpdateTradeZoneBounds(tradeZone_bounds) {
    console.log('NEW_BOUNDS', tradeZone_bounds)
  ...
  loadNewTradeZoneCart = async () => {
    ...
    if (!this.props.isCity) {
      let data = await getTradeZoneCartography(this.props.address.state.toLowerCase(), this.props.tradeZone_bounds)
      let mCartography = Object.assign({}, this.state.cartography)
    ...
    // new pull json from local file
    else {
      let data = getBlockGroups(this.props.tradeZone_bounds)
      let mCartography = Object.assign({}, this.state.cartography)
      mCartography.tradezone = data
      await this.setState({ cartography : mCartography, cartographyLoaded : true, loadingCart : {...this.state.loadingCart, [TRADE_ZONE] : false } })
      this.renderCartography()
  ...
  updateRadii() {
   ...
   /*   
        let points1 = getPoints(this.state.center, 0.35, 40)
        let points2 = getPoints(this.state.center, 0.25, 25)
        let points3 = getPoints(this.state.center, 0.15, 10)
        let points4 = getPoints(this.state.center, 0.08, 5)
      ...
      */
      ...
      let points1 = getPoints(this.state.center, 4.628, 50)
      let points2 = getPoints(this.state.center, 4.2, 10)
      let points3 = getPoints(this.state.center, 3.8, 50)
      let points4 = getPoints(this.state.center, 3.4, 10)
      let points5 = getPoints(this.state.center, 3.0, 40)
      let points6 = getPoints(this.state.center, 2.6, 10)
      let points7 = getPoints(this.state.center, 2.2, 30)
      let points8 = getPoints(this.state.center, 1.5, 10)
      let points9 = getPoints(this.state.center, 1.0, 10)
      let points = points1.concat(points2).concat(points3).concat(points4).concat(points5)
        .concat(points6).concat(points7).concat(points8).concat(points9)
      console.log(points)
      points.forEach((e, i) => {
        renderMarker(i, e, this.state.map, e.lat + ', ' + e.lng, GREEN_MARKER)
      })
      ...
      let outerRing = getPoints(this.state.center, 8.046, 80)
      outerRing.forEach((e,i) => {
        renderMarker(i, e, this.state.map, e.lat + ', ' + e.lng, RED_MARKER)
      })
