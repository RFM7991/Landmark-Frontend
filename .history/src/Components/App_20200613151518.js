...
class App extends React.Component { 
  ...
  //  localStorage.clear()
   ...
   // cookies.remove('myCat')
    console.log('cookies', cookies.get('firstTimeUser'));
    if (cookies.get('firstTimeUser') == undefined && user._id == -1 ) {
      this.setState({ isFirstTimeUser : true  })
      cookies.set('firstTimeUser', 1, { path: '/', expires : new Date('2200-12-1T03:24:00')})
     } 
     else {
    ...
    if (this.props.ready) {
      ...
      var map = <div>
        <div className="results-container">
          ...
          <SimpleMap runJoyRideTutorial={this.runJoyRideTutorial} height={'72.2vh'} address={this.props.address} center={this.props.address.coords} zoom={15} business_type={this.props.business_type} isFirstTimeUser={this.state.isFirstTimeUser}/>
          <PlacesList/>
          </div>
          <br></br>
          <ChartsPanel/>
