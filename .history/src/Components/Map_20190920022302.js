...
class SimpleMap extends Component {
  ...
  onHandleAerial = () => {
    ...
    console.log('aerial?', this.state.aerial, 'map:', this.state.map)
    if (this.state.aerial) {
      this.setState({ zoom : 18, mapType : google.maps.MapTypeId.SATELLITE })
      this.setState({ mapOptions : this.updateMapOptions})
    } else {
      this.setState({ zoom : this.props.zoom, mapType : google.maps.MapTypeId.SATELLITE })
      this.setState({ mapOptions : this.updateMapOptions})
    }
  }
