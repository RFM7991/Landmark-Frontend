...
class SimpleMap extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    // update geojson
    if (this.state.map !== undefined) {
      console.log(this.props.data_range)
      // clear data layer 
      ...
      if (this.props.data_range == ZIP) {
        if (this.state.cartography.zip !== null)
          this.state.cartography.zip.forEach(featureSet => {
            this.state.map.addGeoJson(featureSet)
          })
      } else if (this.props.data_range == TRADE_ZONE) {
        if (this.state.cartography.tradezone !== null)
  ...
  async loadCartography(callback) {
    await getZipCartography(this.props.state.toString().toLowerCase(), this.props.zip).then((data) => {
      let mCartography = Object.assign({}, this.state.cartography)
      mCartography.zip = data
      this.setState({cartography : mCartography})
    })
    await getTradeZoneCartography(this.props.state.toString().toLowerCase(), this.props.center.lat, this.props.center.lng)
      .then((data) => {
        let mCartography = Object.assign({}, this.state.cartography)
