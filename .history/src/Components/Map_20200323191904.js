...
class SimpleMap extends Component {
  ...
  async updateRadii() {
    console.log("COUNTY?", this.props.address)
    let county = getCounty(this.state.center)
      if (this.props.isCity) {
        ...
        const circle2 = renderCircle(circle2Distance, "#0000ff", this.state.map, this.state.center) 
        // to-do cleanup, add label
        let points = getPoints(this.state.center, 1.608, 1)
        renderComplexMarker('circle1', points[0], this.state.map, '1 mi', YELLOW_DOT_3, new google.maps.Point(60, 60))
        points = getPoints(this.state.center, 0.804, 1)
        renderComplexMarker('circle2', points[0], this.state.map, '0.5 mi', BLUE_DOT_05, new google.maps.Point(60, 60))
        ...
        let outerIndex = ((increment*divisor)*8) + (increment*(divisor-1)*7)
