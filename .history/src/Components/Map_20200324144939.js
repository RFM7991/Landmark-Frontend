...
class SimpleMap extends Component {
  ...
  renderCartography(dontClear) {
    // clear data layer 
      if (!dontClear || (this.state.cartography.zip !== undefined && this.state.cartography.tradezone !== undefined)) this.clearCartography()
     console.log("CART_TEST", this.props.data_range, this.state.cartography.zip)
      ...
      if (this.props.data_range == ZIP) {
        ...
        if (this.state.cartography.tradezone !== undefined) {
         ...
         setTimeout(() => this.renderCartography(true), 1000)
       }
    }
