...
class DoughnutChart extends Component {
	...
	getDataPoints = () => {
		...
		switch (this.props.title) {
			case GENDER: 
				let total = 0; let genderStats = stats.social.summary.gender
				Object.entries(genderStats).map(([key,  value]) => { total += value})
				let data = [
					{name: 'Males', y: Math.round(genderStats.males / total)},
					{name: 'Females', y: Math.round(genderStats.females / total)}
				]
