...
class DemographicsPanel extends React.Component {
  ...
  //      .then(res => this.onUpdateTradeZoneBounds(res))
        ...
        let data;
        // zip
        if (this.props.geo_unit == 'zip') {
            let bounds = await getZipTradeZoneBounds(this.props.isCity, this.props.address.coords)
            this.onUpdateTradeZoneBounds(bounds)
            data = await getTradeZoneZipStats(bounds)
        }   
        else {
            let bounds = await getTradeZoneBounds(this.props.isCity, this.props.address.coords)
            this.onUpdateTradeZoneBounds(bounds)

            data = (this.props.isCity) // this.props.isCity
                ? await getTradeZoneBlockStats(bounds)
                : await getTradeZoneStats(bounds)
        }
    ...
    onUpdateTradeZoneBounds = tradeZone_bounds => {
        console.log('NEW_BOUNDS_DEMO', this.props)
