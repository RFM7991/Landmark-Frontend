...
import { getZipCartography, getTradeZoneCartography, getLoadedTradeZoneCartography } from '../Requests/cartography-requests'
  ...
  async loadTradeZoneCartography() {
    if (!this.state.cartographyLoaded) {
      ...
      if (this.props.address.isNewEntry) {
        ...
        let data = await getLoadedTradeZoneCartography(this.props.address.place.place_id, this.props.data_range)
        console.log('LOADED CART', data)
        if (data == undefined) { // if not found load new
          this.loadNewTradeZoneCart()
        } else {
          
          let mCartography = Object.assign({}, this.state.cartography)
          mCartography.tradezone = data.cartography
  ...
  loadNewTradeZoneCart = async () => {
    await getTradeZoneBounds(this.props.isCity, this.props.address.coords).then(res => this.onUpdateTradeZoneBounds(res))
        await getTradeZoneCartography(this.props.address.state.toLowerCase(), this.props.tradeZone_bounds).then( async (data) => {
          ...
          let tzCartUploadResults = await createTradeZoneCartography(this.props.address.place.place_id, data, this.props.data_range)
