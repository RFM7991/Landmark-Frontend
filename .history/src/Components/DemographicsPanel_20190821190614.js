...
class DemographicsPanel extends React.Component {
   ...
   onUpdateRange(event) {
    ...
    getTradeZoneData() {
        let tracts = []
        this.state.boundaries.forEach((e, i) => {
            getTract(e.lat, e.lng, (data) => {
              let tract = data[0].tract
              
              if (!tracts.includes(tract)) {
                    tracts.push(tract)
                    getSocial(TRACT, e.lat, e.lng, (data) => {
                        console.log('Done')
                    })
                    getAge(TRACT, e.lat, e.lng, (data) => {
                        console.log('Done')
                    })
                    getIncome(TRACT, e.lat, e.lng, (data) => {
                        console.log('Done')
                    })
                }
            })
