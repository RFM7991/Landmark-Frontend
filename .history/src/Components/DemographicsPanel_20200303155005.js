...
class DemographicsPanel extends React.Component {
   ...
   //     if (this.props.address.isNewEntry) {
    ...
    loadNewTradeZoneStats = async () => {
        ...
        let data = await getTradeZoneBlockStats(this.props.tradeZone_bounds)
        console.log('BLOCK_DATA', data)
            ...
            <div style={{ flexGrow: 1, overflowY: 'auto', backgroundColor: lightBg}}>
               <Accordion defaultActiveKey='0'>
                ...
                <Card style={{backgroundColor: lightBg}}>
                    ...
                    <Card.Body>
                        <Accordion defaultActiveKey="0">
                        { !(this.props.data_range == TRADE_ZONE && this.props.isCity) && <NestedCard name={"Ethnicity"} data={social_data.race} key_i={"0"}></NestedCard> }
                        { !(this.props.data_range == TRADE_ZONE && this.props.isCity) &&  <NestedCard name={"Nativity"} data={social_data.nativity} key_i={"1"}></NestedCard> }
                        { !(this.props.data_range == TRADE_ZONE && this.props.isCity) && <NestedCard name={"Education (Ages 25+)"} data={social_data.education} key_i={"2"}></NestedCard> }
                        <NestedCard name={"Marital Status (Ages 15+)"} data={social_data.marital_status} key_i={"3"}></NestedCard> 
                        { !(this.props.data_range == TRADE_ZONE && this.props.isCity) && <NestedCard name={"Transportation (for work)"} data={social_data.transportation} key_i={"4"}></NestedCard> }
                        </Accordion>
                    </Card.Body>
                    </Accordion.Collapse>
                </Card>
