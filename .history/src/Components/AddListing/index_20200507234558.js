...
class AddListing extends React.Component {
    ...
    handleSetPhotos = async (category, photo) => {
        let photosState = this.state.photos
        if (photosState[category] == undefined) {
            photosState[category] = []
        }

        photosState[category].push(photos)

        await this.setState({ photos : photosState})

        console.log("MASTER_PHOTOS", this.state.photos)
    }
               ...
               <div style={{ backgroundColor: 'rgba(146, 176, 179, 0.6)', width: '80%', height: '10%', flexDirection: 'column'}}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column',  width: '80%', height: '100%', backgroundColor: 'rgba(255,255,255, 0.7)'}}>
                    ...
                    <div style={(this.state.index == 1) ? {} : { display : 'none'} }> 
                        <ContactInfo addFormInfo={this.addFormInfo} handleNext={this.handleNext} handleSetPhotos={this.handleSetPhotos}/>
                    </div>
                    <div style={(this.state.index == 2) ? {} : { display : 'none'}}> 
                        <LocationDetails addFormInfo={this.addFormInfo}  handleNext={this.handleNext}/>
                    </div>
