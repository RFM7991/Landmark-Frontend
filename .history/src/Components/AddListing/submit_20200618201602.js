    ...
    render() {
        ...
        return (
            <div style={{ display: 'flex', alignItems: 'flex-start',  flexDirection: 'column', padding: '20px' }}>
                ...
                <div style={{ width: '100%', height: '100px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', backgroundColor: 'blue'}}>
                    {this.state.error && <span style={{ color: 'red',  marginBottom: '4em'}}>{this.state.errorMessage}</span>}
                    {this.state.loading &&  <PropogateLoader color='#00D4FF' size={10} />}
                    {!this.state.loading && 
                        <Button variant="primary" onClick={this.handleSubmit} 
                            style={{backgroundColor:'#00d4ff', fontWeight: 'bold',   marginTop: '1em'  }}>
                            Submit
                        </Button>
                    }
                </div>
