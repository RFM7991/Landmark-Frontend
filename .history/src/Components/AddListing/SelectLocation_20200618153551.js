        ...
        let geoCode = await geocodeByAddress(address).then(results => { 
            ...
            if (place != undefined) {
                ...
                if (list_check.length > 0) {
                    this.setState({ error : true, errorMessage : "Sorry, this location already has a listing." })
                    error = true 
                    return;
                }
        ...
        return (
            <div style={{ display: 'flex',  height: '80vh', alignItems: 'center', justifyContent: 'center',  flexDirection: 'column', padding: '20px' }}>
                ...
                <div style={{ flex: 3,  width: '60%' }}>
                    ...
                    <br></br>
                    <Button variant="primary" onClick={this.handleSubmit} disabled={this.state.address == undefined || this.state.error}
