    ...
    render() {
        return (
            <div style={{ display: 'flex', alignItems: 'flex-start',  flexDirection: 'column', padding: '20px' }}>
                ...
                <div style={{ display: 'flex', width: '100%', alignItems: 'flex-start', flexDirection: 'column', marginLeft: '24px', marginTop: '10px' }}>
                ...
                {this.props.formData.locationInfo != undefined &&
                <Table>
                    <th></th>
                    {Object.entries(this.props.formData.locationInfo).map(([key, val], i) => {
                        return (
                            <tr><td>{key}</td><td>{val}</td></tr>
                        )
                    })}
