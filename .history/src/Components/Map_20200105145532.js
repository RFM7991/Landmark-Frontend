...
class SimpleMap extends Component {
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    if (this.props.active_place !== prevProps.active_place) 
      if (this.props.active_place.toString().length > 0) {
        ...
        console.log(prevProps.active_place.id, this.state.markers.get(prevProps.active_place.id))
    ...
    if (this.state.map !== undefined) {
      if (this.props.data_range !== prevProps.data_range) {
        if (this.state.display_cartography)
          await this.renderCartography(true)
      }   
    }
  ...
  renderCartography(clear) {
    console.log('DATA RANGE', this.props.data_range)
    // clear data layer 
    if (clear)
      ...
      if (this.props.data_range == ZIP) {
        ...
        if (this.state.cartography.tradezone !== undefined) {
         ...
         setTimeout(() => this.renderCartography(false), 100)
       }
    }
  }
