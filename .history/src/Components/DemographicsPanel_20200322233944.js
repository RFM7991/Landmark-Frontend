...
import { isCitySelector, tractSelector, addressSelector, dataRangeSelector, tradeZoneBoundsSelector, statsSelector, geoUnitSelector } from '../Reducers/selectors';
...
class DemographicsPanel extends React.Component {
   ...
   //     if (this.props.address.isNewEntry) {
    ...
    loadNewTradeZoneStats = async () => {
        console.log('LOAD NEW STATS', this.props)
        let bounds = await getTradeZoneBounds(this.props.isCity, this.props.address.coords, this.props.geo_unit, true)
        this.onUpdateTradeZoneBounds(bounds)

        
         let data;
        switch (this.props.geo_unit) {
            case 'zip' : data = await getTradeZoneZipStats(bounds); break;
            case 'tract' : data = await getTradeZoneStats(bounds); break;
            case 'block' : data = await getTradeZoneBlockStats(bounds); break;
            default : break;
        }
    ...
    onUpdateTradeZoneBounds = tradeZone_bounds => {
        console.log('NEW_BOUNDS_DEMO', tradeZone_bounds, this.props)
...
const mapStateToProps = createSelector(
    ...
    */
    ...
    (tract, address, data_range, tradeZone_bounds, stats, isCity, geo_unit) => ({
        tract, address, data_range, tradeZone_bounds, stats, isCity, geo_unit
    })
);
