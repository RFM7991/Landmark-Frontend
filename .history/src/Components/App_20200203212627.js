...
class App extends React.Component { 
  ...
  //  localStorage.clear()
    ...
    if (user == undefined) user = {_id: -1, username: 'guest', is_admin: false}
      ...
      userInfo = await getUserInfo(user._id).res[0]
      console.log('USER INFO', userInfo)
      if (userInfo.recentSearches.length > 0) {
        await this.setState({ searches : userInfo.searches})
        localStorage.setItem('recentSearches', JSON.stringify(userInfo.recentSearches))
      }
    }
