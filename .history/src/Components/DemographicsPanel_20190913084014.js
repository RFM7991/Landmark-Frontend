...
class DemographicsPanel extends React.Component {
   ...
   onUpdateDataRange(event) {
    ...
    async getData () {
        let zipStats = await getAllStats(this.props.lat, this.props.lng, 'zip')
        this.setState({ zipStats : zipStats, currentData : zipStats})
        let tradeZoneStats = await getAllStats(this.props.lat, this.props.lng, 'tradezone')
        this.setState({tradeZoneStats : tradeZoneStats})
