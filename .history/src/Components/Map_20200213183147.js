      ...
      if (this.props.address.isNewEntry) {
       ...
       //   mCartography.tradezone = data[0].cartography
         let collection = []
          ...
          this.setState({ cartography : mCartography, cartographyLoaded : true})
  ...
  loadNewTradeZoneCart = async () => {
    ...
    await getTradeZoneBounds(this.props.isCity, this.props.address.coords).then(res => this.onUpdateTradeZoneBounds(res))
        await getTradeZoneCartography(this.props.address.state.toLowerCase(), this.props.tradeZone_bounds).then( async (data) => {
          ...
          await this.setState({ cartography : mCartography, cartographyLoaded : true})
...
generateHeading = () => {
 ...
 //  console.log('map props', this.props)
  ...
  <div className='map-control_bar'>
    <div style={{ display : 'flex', alignItems: 'flex-end', marginLeft: '2.5%', height: '7.5vh'}}>
    <Button style={{ fontSize: 12, marginRight: '1em'}} onPress={this.props.runJoyRideTutorial}>Help</Button>
      <Button style={{ fontSize: 12}} variant="light" onClick={this.onHandleCenter}> 
        Center
      </Button>
      <div style={{ display: 'flex',  justifyContent: 'center',  marginTop: '1em', marginLeft: '10px'}}>
